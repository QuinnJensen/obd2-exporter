...
my @trans_pids = (
	...
	{pid => "2211c0", name => "EPC",   descr => "trans line pressure control",	offset => 0, bits => 8,  scale => 5 / 10, fmt => "%g psi", type => "pressure"},
...
sub do_group {
	...
	# hand format the JSON to set it in order for grafana live view
	...
	for my $p (@$g) {
		...
		if ($status eq "ok" && defined($obd->{results}->{"10"})) {
			...
			update_gauge($p, $val);
			$formatted = sprintf($p->{fmt}, $val);
		} else {
			update_gauge($p, undef);
		}

		printf("%s %-4s %-5s %-16s %s\n", $p->{pid}, $valstr, $p->{name}, $formatted, $p->{descr});
	}
	$json .= "}";
	return $json;
}
